...
Canonical file is repo-root `clip.yaml` (split hosts). Historical snapshot below is **not** the live file.
...
>
> - **`_login.ts` 把 `apiUrl` / `loginPath` / `tokenParam` 全部硬编码进生成的文件**(`../clip/packages/cli/src/codegen/templates.ts:306-332`)。`CLIP_BASE_URL` 只覆盖**普通业务请求**的 baseUrl(`templates.ts:100`),**不影响 `bogo login`**。要让 `bogo login` 打本地 wrangler,必须**生成时就给一份临时 schema**,把 `baseUrl` **和** `loginUrl` 都改成 `http://127.0.0.1:<PORT>`（login 走 `/api/auth/cli`），然后从这份 schema generate
